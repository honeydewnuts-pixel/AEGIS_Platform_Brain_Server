...
class SubscriptionService:
    ...
    async def apply_event(self, event: PaymentEvent) -> None:
        ...
        async with async_session_factory() as session:
            if event.event_type == PaymentEventType.PAYMENT_SUCCEEDED:
                ...
                if is_first_activation:
                    ...
                    self.logger.warning(
                        "First activation for %s - issued mobile API key (shown once): %s",
                        event.account_id, mobile_api_key,
                    )
                    # NOTE: this is logged, not emailed/displayed anywhere yet -
                    # you need to surface this to the subscriber (checkout
                    # success page, email, etc.) same as portal_token. See
                    # README "Known gaps" - no email delivery is wired up.
